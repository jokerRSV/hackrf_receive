...
public class WavFile {
    ...
    // Cannot instantiate mavlib.WavFile directly, must either use newWavFile() or openWavFile()
    private WavFile() {
    ...
    private int readFramesInternal(float[] sampleBuffer, int offset, int numFramesToRead) throws IOException, WavFileException {
        if (ioState != IOState.READING) throw new IOException("Cannot read from mavlib.WavFile instance");
        ...
        for (int f = 0; f < numFramesToRead; f++) {
            ...
            for (int c = 0; c < numChannels; c++) {
                ...
                sampleBuffer[offset] = floatOffset + (float) sample / floatScale;
                offset++;
            }
